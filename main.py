...
algo = cv2.createBackgroundSubtractorMOG2(history=50,varThreshold=25,detectShadows=False)#removes the background of the vehicles 
...
while True:
    ...
    blur=cv2.GaussianBlur(gray,(3,3),5)  #makes the video smoother by reducing sharpness hence decreasing clarity. NOTE: more kernal value means algo will pick up less things(only odd values)
    #Now applying on frames
    Sub_img=algo.apply(blur) #using the algorithm on blur
    dilated_img=cv2.dilate(Sub_img,np.ones((2,2)))  #grows the img by increeasing size of neighobouring pixels
    ...
    for (i, c) in enumerate(contourshape):
        ...
        if w >= minwidthrect and h >= minheightrect:
            ...
            if aspect_ratio < 1.2 or aspect_ratio > 5:
                continue 
    ...
    cv2.putText(frame1,"Vehicles passed:"+str(counter),(470,70),cv2.FONT_ITALIC,2.4,(0,0,0),3)
